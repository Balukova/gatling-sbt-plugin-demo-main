...
class TestSimulation extends Simulation{
  ...
  val scenario2 = scenario("amount of elements on page").exec(
    http("page 3, 40 elems ")
      .get("/computers?p=3&n=40")
      .check( status is 200)
  ...
  val scenario4 = scenario("find be name").exec(
    http("find lenovo")
      .get("/computers?f=lenovo")
  )

  val scenario5 = scenario("crete").exec(
    http("create")
      .post("/computers")
      .formParam("""name""", """lenovo m341fn""")
      .formParam("""introduced""", """2021-01-01""")
      .formParam("""discounted""", """""")
      .formParam("""company""", """1""")
      .check(status is 200)
  ...
  setUp(
    ...
    scenario4.inject(atOnceUsers(1)).protocols(httpProtocol),
    scenario5.inject(atOnceUsers(1)).protocols(httpProtocol)
  )
